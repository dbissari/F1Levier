...
public class GlobalTeamStatsAdapter extends RecyclerView.Adapter<GlobalTeamStatsAdapter.ViewHolder> {

    private List<TeamStat> mTeamStats = Collections.emptyList();
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        final TeamStat currentStat = mTeamStats.get(position);
        holder.rankTextView.setText(String.valueOf(position+1));
        holder.nameTextView.setText(currentStat.name);
        holder.timeTextView.setText(Chronometer.format(currentStat.time));
    }

    void setTeamStats(List<TeamStat> teamStats) {
        mTeamStats = teamStats;
        notifyDataSetChanged();
    }

    @Override
    public int getItemCount() {
        return mTeamStats.size();
    }
